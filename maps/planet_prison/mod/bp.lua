...
public.init = function()
    if global.this == nil then global.this = {} end
...
public.push_blueprint = function(name, bp)
    local entry = {bp = bp, hook = nil, refs = {}}
...
--]]
public.reference_get_bounding_box = function(reference) return reference.bb end

--[[
reference_get_entities - Return references to entities.
@param reference - Valid reference object fetched from get_references.
--]]
public.reference_get_entities =
    function(reference) return reference.entities end

--[[
reference_get_timestamp - Return timestamp of a reference
@param reference - Valid reference object fetched from get_references.
--]]
public.reference_get_timestamp =
    function(reference) return reference.timestamp end
...
public.unlink_references_filtered = function(name, query)
    ...
    for i = #object.refs, 1, -1 do
        ...
        if query and query.timestamp then
            if ref.timestamp > query.timestamp then goto continue end
        end
...
global._bp_destroy_reference = function(surf, ref)
    for _, ent in pairs(ref.entities) do if ent.valid then ent.destroy() end end

    local tiles = {}
    for _, tile in pairs(ref.tiles) do
        if tile.valid then goto continue end
...
local function _build_entities(surf, point, entities, hook, args)
    ...
    for _, ent in pairs(entities) do
        ...
        if not e or not e.valid then goto continue end

        if hook then hook(e, args) end
...
public.build = function(surf, name, point, args)
    ...
    if tiles and #tiles > 0 then
        ...
        local query = {name = "character", area = bb, invert = true}
    ...
    if entities and #entities > 0 then
        instance.entities = _build_entities(surf, point, entities, object.hook,
                                            args)
        ...
        for _, ent_found in pairs(surf.find_entities_filtered(query)) do
            if not ent_found.valid then goto continue end

            for _, ent_spawned in pairs(instance.entities) do
                if ent_found == ent_spawned then goto continue end
            end
