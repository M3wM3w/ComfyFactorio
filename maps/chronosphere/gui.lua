...
local function ETA_seconds_until(chronosteps_endpoint, power, storedbattery) -- power in watts
	local objective = Chrono_table.get_table()
	
	local timespentatchronorate1 = 180
	local n = chronosteps_endpoint - objective.chronotimer

	if n < 0 then return 0
	elseif n < timespentatchronorate1+2 then return n
	else
		-- let's calculate the real-world time it takes to launch if we go at this power
		-- the number of chronosteps we will end up taking, i.e. n, equals the 180 at the end,
		-- plus those generated by accumulators, plus those generated passively.
		-- n = 180 + ((ETA-180)*power+storedbattery)/3MJ + (ETA-180)*passiverate
		-- solve for ETA:
		local eta = (n - timespentatchronorate1 + timespentatchronorate1 * (power/3000000 + objective.passive_charge_rate) - storedbattery/3000000) / (power/3000000 + objective.passive_charge_rate)
...
function Public_gui.update_gui(player)
  ...
  local tick = game.tick
	...
	if tick % 60 == 58 then -- timer updates
		...
		if objective.chronotimer < 8 or objective.chronotimer > objective.chrononeeds - 182 then
			seconds_ETA = ETA_seconds_until(objective.chrononeeds,0,0)
		else
			seconds_ETA = ETA_seconds_until(objective.chrononeeds,powerobserved,storedbattery)
		end
		...
		gui.timer_value.caption = math_floor(seconds_ETA / 60) .. " min, " .. seconds_ETA % 60 .. " s"
			if objective.planet[1].type.id == 19 and objective.passivetimer > 31 then
				...
				local bestcase = 0
				if objective.acumulators then bestcase = math_floor(ETA_seconds_until(objective.chrononeeds,#objective.acumulators * 300000,storedbattery))
				gui.timer2.caption = {"chronosphere.gui_3_1"}
				gui.timer_value2.caption = math_floor(bestcase / 60) .. " min, " .. bestcase % 60 .. " s (drawing " .. #objective.acumulators * 0.3 .. "MW)"
	...
	if objective.chronojumps > 3 then
		local time_until_overstay = (objective.chrononeeds * 0.75 - objective.passivetimer) / 60 / objective.passive_charge_rate
		local time_until_evo = (objective.chrononeeds * 0.5 - objective.passivetimer) / 60 / objective.passive_charge_rate
		local first_part = "Biters permanently evolve when we overstay our welcome: " .. math_floor(time_until_overstay/60) .. " min, " .. math_floor(time_until_overstay) % 60 .. " s"
		if time_until_overstay < 0 then
			first_part = "Biters permanently evolved!"
		end
		...
		if evo_timer_min < 0 then
			second_part = "Evolution ramped up!"
		end
		gui.timer.tooltip = first_part .. "\n" .. second_part --more area is hoverable
		gui.timer_value.tooltip = first_part .. "\n" .. second_part
	else
		gui.timer.tooltip = "From planet 4, biters will get additional permanent evolution for staying too long on each planet."
		gui.timer_value.tooltip = "From planet 4, biters will get additional permanent evolution for staying too long on each planet."
	end
