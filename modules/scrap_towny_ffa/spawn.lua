...
local function get_area(position, w, h)
    local x1 = math.floor(w / 2)
    local x2 = w - x1
    local y1 = math.floor(h / 2)
    local y2 = h - y1
...
local function in_use(position)
    local result = false
    for _, v in pairs(global.towny.spawn_point) do
...
local function find_valid_spawn_point(surface)
    ...
    local keyset = {}
    for town_name, _ in pairs(global.towny.town_centers) do
        table.insert(keyset, town_name)
    end
    local count = table_size(keyset)
    if count > 0 then
        local town_name = keyset[math_random(1, count)]
        local town_center = global.towny.town_centers[town_name]
...
function Public.get_new_spawn_point(player, surface)
    ...
    global.towny.spawn_point[player.name] = position
...
function Public.get_spawn_point(player, surface)
    if global.towny.spawn_point == nil then global.towny.spawn_point = {} end
    -- test the existing spawn point
    local position = global.towny.spawn_point[player.name]
