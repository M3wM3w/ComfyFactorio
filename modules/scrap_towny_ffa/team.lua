...
function is_towny(force)
	if force == game.forces["player"] or force == game.forces["rogue"] then return false end
	return true
end

function Public.give_outlander_plane(player)
	if player == nil then return end
	if player.get_item_count("small-plane") == 0 then
		player.insert({name = "small-plane", count = 1})
	end
end

function Public.remove_plane(entity)
	if entity == nil then return end
	entity.remove_item({name = "small-plane"})
end
...
function Public.set_player_to_outlander(player)
	...
	Public.set_player_color(player)
	Public.give_outlander_plane(player)
end
